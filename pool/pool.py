...
# Add vertical spin to the cue ball
spin_amount = 0
max_spin = 1000
...
#load images
cue_image = pygame.image.load("pool/assets/images/cue.png").convert_alpha()
table_image = pygame.image.load("pool/assets/images/table.png").convert_alpha()
ball_images = []
for i in range(1, 17):
	ball_image = pygame.image.load(f"pool/assets/images/ball_{i}.png").convert_alpha()
...
def create_ball(radius, pos):
	...
	# shape.friction = 0.7 # Add friction coefficient
...
# Make a collision handler
def ball_collision_handler(arbiter, space, data):
	global spin_amount
	if spin_amount != 0:
		ball_a, ball_b = arbiter.shapes
		# Only apply spin to the cue ball
		if ball_a == balls[-1] or ball_b == balls[-1]:
			cue_ball = balls[-1]
			# Get the collision normal and angle
			normal = arbiter.normal
            # Apply velocity in the original direction for topspin
			# Or opposite direction for backspin
			spin_direction = 1 if spin_amount > 0 else -1
			spin_force = abs(spin_amount) * 0.5
			cue_ball.body.velocity += (normal * spin_force * spin_direction)
			# Reset spin after it's been applied
			spin_amount = 0
	return True

# Add the collision handler to the space
collision_handler = space.add_collision_handler(0, 0)
collision_handler.separate = ball_collision_handler
...
while run:
	...
	#event handler (moved to start of loop)
	for event in pygame.event.get():
		if event.type == pygame.MOUSEBUTTONDOWN and taking_shot == True:
			powering_up = True
		if event.type == pygame.MOUSEBUTTONUP and taking_shot == True:
			powering_up = False
		if event.type == pygame.KEYDOWN and taking_shot == True:
			if event.key == pygame.K_t:
				spin_amount = max_spin # Up spin
				print(f"T pressed - spin_amount: {spin_amount}")
			if event.key == pygame.K_b:
				spin_amount = -max_spin # Down spin
				print(f"B pressed - spin_amount: {spin_amount}")
			if event.key == pygame.K_SPACE:
				spin_amount = 0 # Reset spin
				print(f"Space pressed - spin_amount: {spin_amount}")
	...
	# Draw lives counter after spin indicator
	...
	# Draw spin indicator - moved here to be with other bottom panel elements
	print(f"Current spin_amount: {spin_amount}")
	if taking_shot and game_running:
		...
		draw_text(spin_text, font, WHITE, 20, SCREEN_HEIGHT + 10)
